...
import os, redis, re, html, openai, json
...
# 6. 데이터 불러오기(if => redis 에서, else => youtube api key로 호출)
...
def summarize_documents(channel_name, video_title, korean_transcript, thumbnail, publish_time, embed_model):
    ...
    if existing_summary:
        ...
        if documents:
            ...
            #predictor_index = GPTVectorStoreIndex.from_documents(documents,service_context= ServiceContext.from_defaults(llm_predictor=llm_predictor))
            #helper_index = GPTVectorStoreIndex.from_documents(documents, service_context= ServiceContext.from_defaults(prompt_helper = prompt_helper))
            result = selected_index.as_query_engine().query('텍스트의 내용을 10개의 핵심 문단으로 요약해주세요. 중복 내용이면 한번만 요약 해주세요.')
            #result = predictor_index.as_query_engine().query('텍스트의 내용을 10개의 핵심 문단으로 요약해주세요. 중복 내용이면 한번만 요약 해주세요.')
            #result = helper_index.as_query_engine().query('텍스트의 내용을 10개의 핵심 문단으로 요약해주세요. 중복 내용이면 한번만 요약 해주세요.')
            print(f"문서 요약:\n{result}\n")
            summary = str(result)
...
#prompt_helper = PromptHelper(num_output=256, chunk_overlap_ratio=0.1) # LLM의 최대 출력 토큰수 조정, 10% 비율로 오버랩 가능하게 조정
...
def get_video_info(channel_id, channel_name):
    ...
    try:
        ...
        #summary = summarize_documents(channel_name, video_title, korean_transcript, thumbnail, publish_time, llm_predictor)
        #summary = summarize_documents(channel_name, video_title, korean_transcript, thumbnail, publish_time, prompt_helper)
        # 딕셔너리 생성
        video_info = {
