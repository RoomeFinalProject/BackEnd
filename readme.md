...
# 한계점
    ...
    1. 채널 매핑을 자동으로 지정을 못함(redis와 db에 저장된 내용과 youtube 최신영상의 내용을 비교하고 자동 업데이트 기능에 제약이 있음)
    2. api_key 할당량 때문에 제한적인 부분이 많았고(실시간 자동 업데이트 기능이나 한번에 대용량의 데이터 호출에 제약이 있음)
    3. 자막을 가져올때 불용어나 의미없는 문자들이 자막에 섞여있거나 , 유튜버가 콩글리시를 많이 사용하면 요약에 영향이 있음
    4. 자막을 지원하지 않는 영상은 데이터 접근 자체가 불가능함